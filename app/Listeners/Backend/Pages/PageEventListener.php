...
class PageEventListener
{
    ...
    public function onCreated($event)
    {
        history()->withType($this->history_slug)
            ->withEntity($event->pages->id)
            ->withText('trans("history.backend.pages.created") <strong>'.$event->pages->title.'</strong>')
            ->withIcon('plus')
            ->withClass('bg-green')
            ->log();
    }
    ...
    public function onUpdated($event)
    {
        history()->withType($this->history_slug)
            ->withEntity($event->pages->id)
            ->withText('trans("history.backend.pages.updated") <strong>'.$event->pages->title.'</strong>')
            ->withIcon('save')
            ->withClass('bg-aqua')
            ->log();
    }
    ...
    public function onDeleted($event)
    {
        history()->withType($this->history_slug)
            ->withEntity($event->pages->id)
            ->withText('trans("history.backend.pages.deleted") <strong>'.$event->pages->title.'</strong>')
            ->withIcon('trash')
            ->withClass('bg-maroon')
            ->log();
    }
